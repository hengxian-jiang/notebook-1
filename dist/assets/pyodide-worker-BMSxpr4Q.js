...
def _capture_open_figures():
    ...
    plt.close('all')
    return results
`);try{n.runPython(W)}catch(e){console.warn("Failed to load Strategus spec builder:",e)}try{for(const[e,t]of Object.entries(H)){const a="/home/pyodide/"+e.replace("./",""),l=a.substring(0,a.lastIndexOf("/"));n.runPython(`
import os
...
${e}`)}catch(e){console.warn("Failed to set environment variables:",e)}p=!0,s({type:"ready",id:""}),s({type:"status",id:"",data:{state:"idle"}})}catch(_){s({type:"error",id:"",data:{ename:"InitializationError",evalue:_ instanceof Error?_.message:String(_),traceback:[]}}),s({type:"status",id:"",data:{state:"error"}})}}}function J(r){const i=r.match(/No module named '([^'.]+)'/);return i?i[1]:null}const $={jwt:"PyJWT",yaml:"pyyaml",dotenv:"python-dotenv",cv2:"opencv-python",PIL:"Pillow",sklearn:"scikit-learn",bs4:"beautifulsoup4",attr:"attrs",msal:"msal"};async function X(r,i){if(!n||!p){s({type:"error",id:r,data:{ename:"NotInitializedError",evalue:"Pyodide is not initialized",traceback:[]}});return}c=r,s({type:"status",id:r,data:{state:"busy"}});try{try{n.globals.set("__user_code__",i);const e=n.runPython("from pyodide.code import find_imports as _fi; list(_fi(__user_code__))"),t=e.toJs();e.destroy(),n.globals.delete("__user_code__"),t.length>0&&await n.loadPackage(t)}catch{await n.loadPackagesFromImports(i)}try{n.runPython(`
try:
    import matplotlib
    matplotlib.use('agg')
except ImportError:
    pass
`)}catch{}let o;const _=5;for(let e=0;;e++)try{o=await n.runPythonAsync(i);break}catch(t){const a=t instanceof Error?t.message:String(t),l=J(a);if(!l||e>=_)throw t;const d=$[l]||l;s({type:"stderr",id:r,data:`Installing ${d}...`});try{await n.loadPackage(d)}catch{try{await n.runPythonAsync(`import micropip; await micropip.install("${d}")`)}catch{throw t}}}if(o!=null){const e=String(o);if(!e.includes("matplotlib.figure.Figure")){let t=e;try{t=n.runPython(`repr(${i.split(`
`).pop()})`)||e}catch{}s({type:"result",id:r,data:{"text/plain":t}})}}try{const e=await n.runPythonAsync("_capture_open_figures()"),t=e.toJs();for(const a of t)s({type:"display_data",id:r,data:{"image/png":a}});e.destroy()}catch{}s({type:"status",id:r,data:{state:"idle"}})}catch(o){const _=o instanceof Error?o.message:String(o),e=o instanceof Error&&o.stack?o.stack.split(`
`):[];s({type:"error",id:r,data:{ename:o instanceof Error?o.constructor.name:"Error",evalue:_,traceback:e}}),s({type:"status",id:r,data:{state:"idle"}})}finally{c=null}}self.onmessage=async r=>{const{type:i,id:o,code:_,indexUrl:e,preloadPackages:t,envVars:a}=r.data;switch(i){case"init":await K(e,t,a);break;case"execute":_&&await X(o,_);break}};
